...
export default function Timing() {
  ...
  const formatTime = (ms: number) => {
    ...
    if (hours > 0) {
      return `${hours}:${pad(minutes)}:${seconds.toFixed(1).padStart(4, '0')}`;
    } else if (minutes > 0) {
  ...
  useEffect(() => {
    const updateTimer = () => {
      ...
      setDisplayTime(formatTime(elapsed));
      if (startTime != null) {
        AsyncStorage.setItem(TIMER_KEY, JSON.stringify({ isRunning: true, startTime, elapsedBeforePause: 0 }));
      }
    };
  ...
  const handleStopwatchPress = () => {
    ...
    if (!isRunning) {
      ...
      setIsRunning(true);
      AsyncStorage.setItem(TIMER_KEY, JSON.stringify({ isRunning: true, startTime: newStartTime, elapsedBeforePause: 0 }));
    } else {
      const now = Date.now();
      const totalElapsed = now - (startTime ?? now);
      setElapsedBeforePause(totalElapsed);
      setTimingData((prev) => [...prev, { place: prev.length + 1, time: formatTime(totalElapsed) }]);
    }
  };
  ...
  const backgroundColor = flashAnim.interpolate({ inputRange: [0, 1], outputRange: ["orange", "#00FF00"] });
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={{ flex: 1, backgroundColor: '#ffffff' }}>
        <View style={styles.titleBar}>
          ...
          <Pressable onPress={handleSettingsPress} style={styles.settingsButton}>
            <Image
              source={
                isRunning
                  ? require("./images/black_circle.png")
                  : require("./images/trash-can-icon-3.png")
              }
        ...
        <View style={{ flex: 1 }}>
          <View style={{ flex: 1 }}>
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4, paddingHorizontal: 20 }}>
                  <Text style={{ fontSize: 18, fontWeight: '500', flex: 1 }}>{item.place}</Text>
                  <Text style={{ fontSize: 18, flex: 1 }}>{item.time}</Text>
                </View>
              )}
        ...
        <TouchableWithoutFeedback onPress={handleStopwatchPress}>
          <Animated.View style={{ flex: 1, backgroundColor, justifyContent: 'center', alignItems: 'center', width: '100%' }}>
            <Text style={[styles.buttonText, { fontSize: 32 }]}>{displayTime}</Text>
          </Animated.View>
        </TouchableWithoutFeedback>

        {!isRunning && (
          <Pressable onPress={exportToCSV} style={styles.backTab}>
            <Text style={styles.backTabText}>Export as CSV</Text>
          </Pressable>
        )}
      </View>
    </KeyboardAvoidingView>
